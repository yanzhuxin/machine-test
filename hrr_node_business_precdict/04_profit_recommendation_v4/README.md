# 04｜存量节点业务利润推荐 v4.1 / v4.3

本目录包含 v4.1 可交付版本及 v4.3 相似节点软伪样本实验版本，包含模型、推理代码、运行说明和结果快照；不包含内网导出的原始数据和鉴权信息。

v4.3 最佳配置、验证结果和未来测试集结果见 [v4_3_soft_pseudo/README.md](v4_3_soft_pseudo/README.md)。v4.3 在纯未见业务上的 Pairwise Accuracy 为 `67.46%`，高于 v4.2 的 `62.72%`；由于全部多业务节点结果低于 v4.2，当前建议仅在纯未见业务场景使用。
...
详细结构与验证指标见 [MODEL_CARD.md](MODEL_CARD.md)；从输入特征、训练样本到成本、收入、毛利和Pairwise排序的逐步讲解见 [完整流程讲解.md](完整流程讲解.md)。
